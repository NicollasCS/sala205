...
// ===== POPUP FUNCTIONS (MUST BE DEFINED EARLY) =====
...
function carregarModoDark() {
    const modoDarkArmazenado = localStorage.getItem('darkMode') === 'true';
    const prefersDark = window.matchMedia('(prefers-color-scheme: dark)').matches;
    const ativoDark = modoDarkArmazenado !== false && (modoDarkArmazenado || prefersDark);
    ...
    if (btn) {
        ...
        if (icon) {
            icon.className = ativoDark ? 'fas fa-sun' : 'fas fa-moon';
        }
        if (text) {
            text.textContent = ativoDark ? 'Claro' : 'Escuro';
        }
    }
}
...
function atualizarStatusLogin() {
  ...
  if (!welcomeEl) {
    ...
    welcomeEl.className = 'welcome-box';
    ...
    welcomeEl.addEventListener('mousedown', (e) => {
      isDragging = true;
      offsetX = e.clientX - welcomeEl.getBoundingClientRect().left;
      offsetY = e.clientY - welcomeEl.getBoundingClientRect().top;
      welcomeEl.style.cursor = 'grabbing';
    });
    ...
    document.addEventListener('mousemove', (e) => {
      if (isDragging) {
        ...
        const maxLeft = window.innerWidth - welcomeEl.offsetWidth;
        const maxTop = window.innerHeight - welcomeEl.offsetHeight;
        
        welcomeEl.style.left = Math.max(0, Math.min(newLeft, maxLeft)) + 'px';
        welcomeEl.style.top = Math.max(0, Math.min(newTop, maxTop)) + 'px';
        welcomeEl.style.right = 'auto';
      }
    });
    
    document.addEventListener('mouseup', () => {
      isDragging = false;
      welcomeEl.style.cursor = 'grab';
    });
  ...
  if (usuarioRaw) {
    ...
    const isMinimized = localStorage.getItem('welcomeMinimized') === 'true';
    ...
    const minimizeBtn = `<button class="welcome-minimize-btn" onclick="toggleWelcomeMinimize()" title="Minimizar/Expandir" style="background: none; border: none; color: white; cursor: pointer; font-size: 1.2rem; margin-left: 0.5rem; padding: 0; width: 24px; height: 24px; display: flex; align-items: center; justify-content: center;">
      <i class="fas fa-${isMinimized ? 'chevron-down' : 'chevron-up'}"></i>
    </button>`;
    ...
    if (isMinimized) {
      welcomeEl.innerHTML = `<div style="display: flex; align-items: center; gap: 0.5rem;">
        <span title="${escapedName}" style="font-size: 1.5rem;">👋</span>
        ${minimizeBtn}
      </div>`;
      welcomeEl.style.padding = '0.8rem 1rem';
      welcomeEl.style.borderRadius = '50px';
    } else {
      welcomeEl.innerHTML = `<div style="display: flex; align-items: center; justify-content: space-between; gap: 1rem;">
        <span>👋 Bem-vindo, <strong>${escapedName}</strong>!</span>
        <div style="display: flex; gap: 0.3rem;">
          ${configBtnHTML}
          ${minimizeBtn}
        </div>
      </div>`;
      welcomeEl.style.padding = '1.2rem 2rem';
    }
    ...
    // Admin button (primeira seção - reatribuição, sem let)
    ...
    // Usar configMenu já declarado anteriormente
    configMenu = document.getElementById('configMenu');
    if (configMenu) {
      configMenu.style.display = 'none'; // Garantir que está escondido
    }

    // Usar alunoCalendarioBtn já declarado anteriormente
    alunoCalendarioBtn = document.getElementById('alunoCalendarioBtn');
    if (alunoCalendarioBtn) {
      if (u.nome === 'aluno205-1') {
        alunoCalendarioBtn.style.display = 'block';
      } else {
        alunoCalendarioBtn.remove();
      }
    }
...
window.toggleWelcomeMinimize = function() {
  const isMinimized = localStorage.getItem('welcomeMinimized') === 'true';
  localStorage.setItem('welcomeMinimized', isMinimized ? 'false' : 'true');
  atualizarStatusLogin(); // Recarregar o elemento
};
...
async function carregarGaleria(page = 0) {
  try {
    ...
    // Mostrar/esconder botão "Próxima página"
    if (nextBtn) {
      nextBtn.style.display = pagination.hasNext ? 'block' : 'none';
    }
    ...
    if (botaoConhecer) {
      // Botão fica escondido por padrão agora
      botaoConhecer.style.display = 'none';
    }
    if (secaoTurma) {
      // Mostrar descrição na página 0, esconder nas outras
      if (galeriaAtualPage === 0) {
        secaoTurma.classList.add('visible');
      } else {
        secaoTurma.classList.remove('visible');
      }
    }
  } catch (e) {
